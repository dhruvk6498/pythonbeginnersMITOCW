...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        newHandDict = {}
        for x in self.handDict.keys():
            lettercount = 0
            lettercount = word.count(x)
            newHandDict[x] = int(self.handDict[x]) - lettercount
        for key in newHandDict.copy().keys():
            if newHandDict[key] == 0:
                del newHandDict[key]
    ...
    def containsLetters(self, letters):
        ...
        isvalid = False
        handkeys = []
        for y in self.handDict.keys():
            handkeys.append(str(y))
        for x in letters:
            if str(x) in handkeys:
                if letters.count(x) <= int(self.handDict[x]):
                    isvalid = True
                else:
                    isvalid = False
                    break
    ...
    def __eq__(self, other):
        ...
        res = False 
        for x in self.handDict.keys():
            if x in other.handDict.keys():
                if self.handDict[x] == other.handDict[x]:
                    res = True
            else:
                res = False
        return res
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for x in word_list:
            if self.hand.containsLetters(x):
                hand_words[x] = int(getWordScore(x))
        if bool(hand_words):
